...
public sealed class AnimationStateMachineTests
{
    ...
    public void ResControllerContainsControllableLocomotionParameters()
    {
        ...
        Assert.That(controller, Is.Not.Null, "Animator Controller should be generated under Assets/Res.");
    ...
    [Test]
    public void ResControllerUsesAvailableCh12MotionClips()
    {
        AnimatorController controller = AssetDatabase.LoadAssetAtPath<AnimatorController>(ControllerPath);
        ...
        AnimatorState jump = controller.layers[0].stateMachine.states
            .Select(state => state.state)
            .FirstOrDefault(state => state != null && state.name == "Jump");

        Assert.That(jump, Is.Not.Null);
        Assert.That(jump.motion, Is.Not.Null);
        Assert.That(jump.motion.name, Is.EqualTo("Jump"));
    }
    ...
    {
        EditorSceneManager.OpenScene("Assets/1.unity", OpenSceneMode.Single);
    }
}
